...
# `resources:` tree in _quarto.yml — so anything under assets/vendor/ that is
# only referenced from inside JavaScript is silently dropped and 404s on the
# deployed site while working perfectly under `quarto preview`. Pushing _site/
# verbatim cannot drift.
set -euo pipefail
cd "$(dirname "$0")/.."
